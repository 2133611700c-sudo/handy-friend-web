...
| 1.5 | Agent A | DONE | `api/ai-chat.js`, `api/ai-intake.js`, `api/alex-webhook.js`, `api/process-outbox.js`, `lib/lead-pipeline.js`, `lib/telegram/send.js` | All 12 raw `fetch('https://api.telegram.org/...')` bypass sites migrated to `lib/telegram/send.js`. `grep "api.telegram.org" api/ lib/` returns only: health.js diagnostic (`getWebhookInfo` read-only), telegram-webhook.js (inbound webhook — out of send scope), send.js base URL const. `lib/telegram/send.js` extended with multipart photo support (coerceToBuffer for buffer/dataUrl inputs). `node --check` OK on all 6 files. Every outbound send now writes durable `telegram_sends` row. | pending PR | 2026-04-17T08:10 |
