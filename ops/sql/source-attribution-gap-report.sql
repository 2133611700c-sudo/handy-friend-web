...
select
  ...
  case
    ...
    when source_details::text ilike '%gclid%' and coalesce(lead_source, to_jsonb(lead_operational_view)->>'source') <> 'google_ads_search' then 'gclid_not_mapped_to_google_ads_search'
    when source_details::text ilike '%utm_source%google%' and coalesce(lead_source, to_jsonb(lead_operational_view)->>'source') is null then 'google_utm_missing_source'
    else 'ok_or_review'
