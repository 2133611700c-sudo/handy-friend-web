...
async function sendLeadCapturedToTelegram({ sessionId, leadId, lang, userText, aiReply, photos, lead }) {
  ...
  const send = await unifiedTelegramSend({
    ...
    chatId,
    timeoutMs: 4000,
    extra: { step: 'lead_captured_legacy', photo_count: photoCount }
  });
  if (!send.ok) {
    throw new Error(send.errorDescription || `sendMessage failed (${send.errorCode || 'unknown'})`);
  }
...
async function sendPreLeadPhotoToTelegram({ sessionId, lang, userText, photos, serviceInference }) {
  ...
  await unifiedTelegramSend({
    ...
    chatId,
    timeoutMs: 4000,
    extra: { step: 'pre_lead_photo_preface', service, confidence, photo_count: photos.length }
  }).catch(() => {});
...
async function sendTelegramPhoto(token, chatId, photo, { caption = '' } = {}) {
  ...
  const send = await unifiedTelegramPhotoSend({
    ...
    chatId,
    timeoutMs: 4000,
    extra: { step: 'photo_forward', name: sanitizeName(photo.name) }
  });
  if (!send.ok) {
    return {
      ok: false,
