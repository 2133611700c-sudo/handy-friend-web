 ...
 */
async function generateAlexWhatsAppReply({ inboundText, customerPhone, conversationHistory = [], hasContact = false, hasPhone = false, extraContext = '' } = {}) {
  ...
  // extraContext carries collected-fields + missing-fields injected by the webhook.
  // It is forwarded to core.js which appends it to the system prompt so Alex
  // knows what the customer already provided and what to ask next.
  const r = await generateAlexReply({ channel: 'whatsapp', messages, hasContact, hasPhone, extraContext });
  return {
    ok: true,
