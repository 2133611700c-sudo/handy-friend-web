...
// Stable hash of the system prompt — proves which prompt was used at call time.
...
async function generateAlexWhatsAppReply({ inboundText, customerPhone, conversationHistory = [] }) {
  ...
  if (flags.length === 0 && modelReply && model === 'deepseek') {
    return {
      ...
      audit: { ...(modelAudit || {}), prompt_version: PROMPT_VERSION, fallback_used: false, source: 'model' },
    };
  }

  // Either DeepSeek failed (model='static_fallback') or output failed safety.
  return {
    ok: true,
    ...
    reason: modelError
      ...
      : (model === 'static_fallback' ? 'deepseek_failed' : `safety_flags: ${flags.join(',')}`),
    safetyFlags: flags,
    needsOwnerApproval: true,
    audit: { ...(modelAudit || {}), prompt_version: PROMPT_VERSION, fallback_used: true, source: 'fallback' },
  };
}
