...
async function handleWAApprovalCallback(update, res) {
  ...
  try {
    ...
    if (!ctx) {
      ...
      // STRICT MODE: the customer-facing text MUST equal the text the operator
      // approved in Telegram. Never substitute. If the stored draft is unsafe
      // or empty, REFUSE to send and tell the operator to run the regen tool.
      const { detectSafetyFlags } = require('../alex/whatsapp-reply-engine.js');
      const flags = detectSafetyFlags(replyText);
      if (flags.length > 0) {
        result = { ok: false, error: 'unsafe_draft', safetyFlags: flags };
        alert = `❌ DRAFT BLOCKED (${flags.join(',')}). Run regen script — do not approve this message again.`;
        // surface to result block; Telegram answerCallbackQuery is sent below.
      } else if (!to) {
