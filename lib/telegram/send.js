...
async function sendTelegramMessage(params) {
  ...
  const retries = Number.isFinite(params.retries) ? Math.max(1, Number(params.retries)) : DEFAULT_RETRIES;
  ...
  if (!text) {
    return { ok: false, messageId: null, errorCode: 'empty_text', errorDescription: 'text is empty', telegramSendId: null };
  }
  ...
  if (networkError) {
    ...
    const logId = await logSendAttempt({
      ...
      requestExcerpt: text.slice(0, 200), extra: { ...extra, retries }
    });
...
/**
 * Send a Telegram photo (data URL or URL or file_id) and log the attempt.
 * For simpler callers. Prefer sendTelegramMessage for text.
 */
async function sendTelegramPhoto(params) {
  ...
  if (!token || !chatId || !photo) {
    return { ok: false, messageId: null, errorCode: 'bad_args', errorDescription: 'missing token/chatId/photo', telegramSendId: null };
  }

  const ctl = new AbortController();
  const timer = setTimeout(() => ctl.abort(), timeoutMs);
  try {
    const isDataUrlPhoto = typeof photo === 'string' && /^data:image\/[a-zA-Z0-9.+-]+;base64,/.test(photo);
    let resp;

    if (isDataUrlPhoto) {
      const parsed = parseDataUrlPhoto(photo);
      if (!parsed.ok) {
        return {
          ok: false,
          messageId: null,
      ...
      form.append('photo', new Blob([parsed.buffer], { type: parsed.mimeType }), parsed.fileName);
...
function parseDataUrlPhoto(dataUrl) {
  ...
  if (parts.length !== 2) {
    return { ok: false, errorCode: 'invalid_data_url', errorDescription: 'photo data URL is malformed' };
  }
  ...
  if (!mimeType || !['image/jpeg', 'image/png', 'image/webp'].includes(mimeType)) {
    return { ok: false, errorCode: 'unsupported_mime', errorDescription: 'photo mime type is not supported' };
  }
  let buffer;
  try {
    buffer = Buffer.from(b64, 'base64');
  } catch (_err) {
    return { ok: false, errorCode: 'invalid_base64', errorDescription: 'photo payload is not valid base64' };
  }
  if (!buffer.length || buffer.length > 8 * 1024 * 1024) {
    return { ok: false, errorCode: 'invalid_size', errorDescription: 'photo payload is empty or too large' };
  }
  const ext = mimeType === 'image/png' ? 'png' : mimeType === 'image/webp' ? 'webp' : 'jpg';
  return { ok: true, buffer, mimeType, fileName: `photo.${ext}` };
}
