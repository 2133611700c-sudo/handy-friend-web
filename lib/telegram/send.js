...
async function sendTelegramPhoto(params) {
  ...
  if (!token || !chatId || !photo) {
    const logId = await logSendAttempt({
      source, leadId, sessionId, chatId: chatId || null, ok: false, messageId: null,
      errorCode: 'bad_args', errorDescription: 'missing token/chatId/photo',
  ...
  try {
    const normalizedPhoto = typeof photo === 'string'
      ? { dataUrl: photo, name: params.fileName || 'photo.jpg' }
      : photo;
    const dataUrl = typeof normalizedPhoto?.dataUrl === 'string' ? normalizedPhoto.dataUrl : '';
    const isDataUrl = dataUrl.startsWith('data:image/');
    ...
    if (isDataUrl) {
      ...
      if (!parsed.ok) {
        const logId = await logSendAttempt({
          source, leadId, sessionId, chatId, ok: false, messageId: null,
          errorCode: 'invalid_data_url', errorDescription: parsed.error,
          requestExcerpt: `PHOTO: ${caption.slice(0, 150)}`,
          extra: { method: 'sendPhoto', mode: 'multipart' }
        });
        return { ok: false, messageId: null, errorCode: 'invalid_data_url', errorDescription: parsed.error, telegramSendId: logId };
      }
      ...
      form.append('photo', new Blob([parsed.buffer], { type: parsed.mimeType }), sanitizePhotoName(normalizedPhoto?.name));
...
function parseImageDataUrl(dataUrl) {
  if (!dataUrl || typeof dataUrl !== 'string') return { ok: false, error: 'empty_data_url' };
  const parts = dataUrl.split(',');
  if (parts.length !== 2) return { ok: false, error: 'invalid_data_url_format' };
