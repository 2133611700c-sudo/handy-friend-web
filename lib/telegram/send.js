...
async function sendTelegramPhoto(params) {
  ...
  const timer = setTimeout(() => ctl.abort(), timeoutMs);
  try {
    const req = buildSendPhotoRequest({ chatId, photo, caption: caption.slice(0, 1024) });
    const resp = await fetch(`${TG_API_BASE}/bot${token}/sendPhoto`, {
      method: 'POST',
...
function buildSendPhotoRequest({ chatId, photo, caption }) {
  if (typeof photo === 'string' && /^data:image\/[a-zA-Z0-9.+-]+;base64,/.test(photo)) {
    ...
    form.append('photo', new Blob([buffer], { type: mimeType }), `photo.${fileExt}`);
    return { body: form };
  }
  return {
    headers: { 'Content-Type': 'application/json' },
