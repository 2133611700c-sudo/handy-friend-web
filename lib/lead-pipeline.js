...
async function createOrMergeLead(leadData) {
  ...
  const run = (async () => {
  const { phone, email, session_id, source, source_details, service_type, name, message, ga4ClientId, is_test } = leadData;
...
async function postLeadWithSchemaFallback(payload) {
  ...
  for (let i = 0; i < 3; i += 1) {
    try {
      ...
      if (/source_details/i.test(message) && Object.prototype.hasOwnProperty.call(nextPayload, 'source_details')) {
        delete nextPayload.source_details;
        continue;
      }
      if (/is_test/i.test(message) && Object.prototype.hasOwnProperty.call(nextPayload, 'is_test')) {
        delete nextPayload.is_test;
        continue;
      }
      throw err;
    }
