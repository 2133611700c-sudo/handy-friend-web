...
async function dispatchOutboundJob(job) {
  if (job.job_type === 'telegram_owner') {
    ...
    const res = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
      ...
      body: JSON.stringify({ chat_id: chatId, text: job.payload?.text || '(no text)', parse_mode: 'HTML', disable_web_page_preview: true })
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || !data.ok) return { ok: false, error: `TG ${res.status}: ${data.description || 'unknown'}`, error_code: `TG_${data.error_code || res.status}` };
