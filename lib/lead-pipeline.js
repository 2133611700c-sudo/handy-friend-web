...
async function dispatchOutboundJob(job) {
  if (job.job_type === 'telegram_owner') {
    ...
    const result = await unifiedTelegramSend({
      source: 'lead_pipeline:dispatch_inline',
      leadId: job.lead_id ? String(job.lead_id) : null,
      text: job.payload?.text || '(no text)',
      chatId,
      token,
      replyMarkup: job.payload?.reply_markup || null,
      extra: { job_id: job.id, job_type: 'telegram_owner' }
    });
    if (!result.ok) {
...
async function enqueueSideEffects(lead, envelope, isNew) {
  ...
  if (canAlert) {
    ...
    await enqueueOutboundJob('telegram_owner', lead.id, { text }, dedupKey)
      ...
      .catch(() => {
        // outbound_jobs not yet available — fire direct fallback via unified sender
        const token  = process.env.TELEGRAM_BOT_TOKEN;
        const chatId = process.env.TELEGRAM_CHAT_ID;
        if (token && chatId) {
          unifiedTelegramSend({
            source: 'lead_pipeline:fallback_owner_alert',
            leadId: lead.id ? String(lead.id) : null,
            text,
            chatId,
            token,
            extra: { is_new: Boolean(isNew), reason: 'outbox_unavailable' }
          }).then(() => markOwnerAlerted(lead.id).catch(() => {})).catch(() => {});
        }
      });
  }
