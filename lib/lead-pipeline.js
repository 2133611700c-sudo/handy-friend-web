...
async function createOrMergeLead(leadData) {
  ...
  const {
    ...
    landing_page, lead_type, attribution_source
  } = leadData;
  ...
  if (utm_content) insertPayload.utm_content = String(utm_content);
  if (utm_term) insertPayload.utm_term = String(utm_term);
  if (gclid) insertPayload.gclid = String(gclid);
  if (gbraid) insertPayload.gbraid = String(gbraid);
  if (wbraid) insertPayload.wbraid = String(wbraid);
  if (fbclid) insertPayload.fbclid = String(fbclid);
  if (msclkid) insertPayload.msclkid = String(msclkid);
  if (landing_page) insertPayload.landing_page = String(landing_page);
  if (lead_type) insertPayload.lead_type = String(lead_type);
 ...
 */
const SMOKE_SOURCE_PATTERN = /^(smoke|canary|test_runtime|verification|test_|pricing_canary)/i;

async function enqueueOutboundJob(jobType, leadId, payload, idempotencyKey) {
  // Suppress outbox jobs for smoke/canary/test leads — prevents DLQ pollution
  const payloadSource = String(payload?.source || payload?.lead_id || idempotencyKey || '');
  if (SMOKE_SOURCE_PATTERN.test(payloadSource)) {
    console.log(`[lead-pipeline] Smoke suppression: skipping ${jobType} job for source=${payloadSource}`);
    return;
  }
...
async function processInbound(envelope) {
  ...
  const result = await createOrMergeLead({
    ...
    utm_content:      envelope.attribution?.utm_content,
    utm_term:         envelope.attribution?.utm_term,
    gclid:            envelope.attribution?.gclid,
    gbraid:           envelope.attribution?.gbraid,
    wbraid:           envelope.attribution?.wbraid,
    fbclid:           envelope.attribution?.fbclid,
    msclkid:          envelope.attribution?.msclkid,
    landing_page:     envelope.attribution?.landing_page,
    attribution_source: envelope.attribution?.attribution_source || envelope.attribution?.referrer || envelope.source,
    source_details:   envelope.attribution?.source_details
  });
