...
async function dispatchOutboundJob(job) {
  if (job.job_type === 'telegram_owner') {
    const send = await sendTelegramMessage({
      ...
      sessionId: job.payload?.session_id || null,
      text: job.payload?.text || '(no text)',
      timeoutMs: 4000,
      extra: { outbox_job_type: 'telegram_owner', outbox_job_id: job.id || null }
    });
    if (!send.ok) {
      const ec = String(send.errorCode || 'unknown');
      let errorCode = `TG_${ec.toUpperCase()}`;
      if (/^\d+$/.test(ec)) errorCode = `TG_HTTP_${ec}`;
      if (ec === 'timeout') errorCode = 'TG_TIMEOUT';
      if (ec === 'network') errorCode = 'TG_NETWORK';
      return { ok: false, error: send.errorDescription || 'telegram_send_failed', error_code: errorCode };
    }
    ...
    await enqueueOutboundJob('telegram_owner', lead.id, { text }, dedupKey)
      ...
      .catch(() => {
        ...
        sendTelegramMessage({
          ...
          text,
          timeoutMs: 4000,
          extra: { fallback_direct: true }
        }).then((result) => {
          if (result?.ok) return markOwnerAlerted(lead.id).catch(() => {});
          return null;
        }).catch(() => {});
      });
  }
