...
async function dispatchOutboundJob(job) {
  if (job.job_type === 'telegram_owner') {
    ...
    if (!send.ok) return { ok: false, error: `TG ${send.errorCode}: ${send.errorDescription || 'unknown'}`, error_code: `TG_${send.errorCode || 'unknown'}` };
      ...
      .catch(() => {
        ...
        if (token && chatId) {
          sendTelegramMessage({
            ...
            chatId,
            timeoutMs: 4000,
            extra: { step: 'enqueue_fallback_direct_send' }
          }).then((send) => {
            if (send?.ok) {
              return markOwnerAlerted(lead.id).catch(() => {});
            }
          }).catch(() => {});
        }
      });
