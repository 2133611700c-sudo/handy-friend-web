...
function pickClickId(safe, clickId, key) {
  return asString(clickId[key] || safe[key]);
}
...
function normalizeAttribution(raw) {
  ...
  const normalized = {
    ...
    clickId: {
      fbclid: pickClickId(safe, clickId, 'fbclid'),
      gclid: pickClickId(safe, clickId, 'gclid'),
      msclkid: pickClickId(safe, clickId, 'msclkid'),
      ttclid: pickClickId(safe, clickId, 'ttclid'),
      gbraid: pickClickId(safe, clickId, 'gbraid'),
      wbraid: pickClickId(safe, clickId, 'wbraid'),
      twclid: pickClickId(safe, clickId, 'twclid'),
      li_fat_id: pickClickId(safe, clickId, 'li_fat_id')
    }
  };
...
function buildAttributionSummary(attr) {
  ...
  if (attr.clickId?.twclid) clickLabels.push('twclid');
  if (attr.clickId?.li_fat_id) clickLabels.push('li_fat_id');
...
function buildSourceDetails(attr) {
  return {
    ...
    clickId: {
      ...
      twclid: attr.clickId?.twclid || '',
      li_fat_id: attr.clickId?.li_fat_id || ''
    }
  };
}
