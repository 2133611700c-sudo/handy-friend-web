...
async function recordInbound({ wamid, from, to, body, media, type, contextWamid, pushName, attribution }) {
...
async function recordOutbound({ wamid, to, body, status = 'sent', approvedBy = null, draftText = null, inReplyToWamid = null, source = null, model = null, audit = null }) {
  try {
    return await sbInsert(TABLE, {
      ...
      raw: {
        in_reply_to_wamid: inReplyToWamid || null,
        alex_source: source || null,
        alex_model: model || null,
        alex_audit: audit || null,
      },
    });
  } catch (err) {
...
/**
 * Has an outbound row already been written for this inbound wamid?
 * Used by the auto-reply path for hard idempotency: one inbound = one outbound.
 */
async function hasOutboundFor(inboundWamid) {
  if (!inboundWamid) return null;
  const url = `${SB_URL}/rest/v1/${TABLE}?direction=eq.out&raw->>in_reply_to_wamid=eq.${encodeURIComponent(inboundWamid)}&select=wamid,phone_number,body,created_at&limit=1`;
  try {
...
module.exports = { recordInbound, updateStatus, recordOutbound, hasOutboundFor, TABLE };
